...
public class GetUserDetailUseCase {
    ...
    public Either<GetUserDetailError, GetUserDetailResult> execute(GetUserDetailQuery query) {
        ...
        // Get user's prediction for the effective round
        ...
        List<TeamRank> sortedRanks = submission.getRankings().stream()
        ...
        // Determine if showing a previous round (effective < phase end means current round isn't finalized yet)
        boolean showingPreviousRound = effectiveRound < phase.getTo();

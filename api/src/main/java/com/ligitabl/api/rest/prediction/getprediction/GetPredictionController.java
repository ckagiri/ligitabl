...
@Slf4j
public class GetPredictionController {
    ...
    public ResponseEntity<?> getSeasonPrediction() {
        ...
        return getPredictionUseCase.execute(userId).fold(this::handleError, this::handleSuccess);
    }

    private ResponseEntity<?> handleError(GetPredictionError error) {
        return switch (error) {
            case GetPredictionError.NotFound __ -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of(
                            "error", "SEASON_NOT_FOUND",
                            "message", "No season available"));

            case GetPredictionError.BaselineRankingsMissing e -> ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of(
                            "error", "BASELINE_RANKINGS_MISSING",
                            "message", "Season baseline rankings missing",
                            "season_id", e.seasonId()));

            case GetPredictionError.HasNoCurrentRound e -> ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of(
                            "error", "SEASON_HAS_NO_CURRENT_ROUND",
                            "message", "Season has no current round",
                            "season_id", e.seasonId()));

            case GetPredictionError.CurrentRoundNotFound e -> ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Map.of(
    ...
    private ResponseEntity<?> handleSuccess(GetPredictionResult result) {

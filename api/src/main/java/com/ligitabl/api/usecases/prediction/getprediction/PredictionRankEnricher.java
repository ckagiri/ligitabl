...
@Slf4j
public class PredictionRankEnricher {

    private final TeamRepo teamRepo;

    public List<PredictionRankDto> enrich(List<TeamRank> rankings) {
        try {
            ...
            return PredictionRankDto.listOf(rankings, teamsByCode);
        } catch (Exception e) {
            log.error("Failed to enrich prediction rankings", e);
            return rankings == null
                    ? List.of()
                    : rankings.stream()
                            .map(rank -> PredictionRankDto.builder()
